package com.jclmsoft.voicesms.receivers;
...
import com.jclmsoft.voicesms.CancelCallback;
...
public class OutgoingCallReceiver extends BroadcastReceiver {
	...
	private void showOngoing(String tickerText, String title, String description) {
		...
		if (mgr != null) {
			mgr.notify(VoiceSMS.NOTIFICATION_CALL, OutgoingCallReceiver.buildOngoing(m_context, tickerText, title, description));
		}
	}

	public static Notification buildOngoing(Context context, String tickerText, String title, String description) {
		Notification calling = new Notification(com.jclmsoft.voicesms.R.drawable.contacticon, tickerText, System.currentTimeMillis());
	...
	private void showError(String tickerText, String title, String description, Intent intent) {
		Notification callFailed = new Notification(com.jclmsoft.voicesms.R.drawable.statusicon_error, tickerText, System.currentTimeMillis());
		...
		NotificationManager mgr = (NotificationManager) m_context.getSystemService(Activity.NOTIFICATION_SERVICE);
		if (mgr != null) {
			mgr.notify(VoiceSMS.NOTIFICATION_CALL, callFailed);
		}
	}
	...
	private class CallTask extends AsyncTask<String, Integer, Integer> {
		...
		private void show(int which) {
			clear(VoiceSMS.NOTIFICATION_CALL);
			switch (which) {
				...
				case PROGRESS_CALL_FAILED:
					showError("Call failed!", "Call failed!", "The call could not be completed!", i(VoiceSMS.class));
					break;
			}
		}
